...
python -W ignore -m examples.lab $(cat $ARGFILE) --save_prefix=$SAVE_PREFIX  --resume_mode=standard  --res_file=$RESUME_FILENAME   --prune_trained --prune_trained_pct=$PRUNE_PCT --cuda  --group_prune_strategy=taylor  --output_level=warning --short_test_report   --global_prune_flop_regularize  --global_prune_flop_lambda=1
