...
python -W ignore -m examples.lab $(cat $ARGFILE) --save_prefix=$SAVE_PREFIX  --resume_mode=standard  --res_file=$RESUME_FILENAME   --prune_trained --prune_trained_pct=$PRUNE_PCT   --prune_layer_mode=global  --proxy_context_type=l1reg_context_slimming   --cuda  --output_level=warning --short_test_report
