...
python -W ignore -m examples.lab $(cat $ARGFILE) --save_prefix=$SAVE_PREFIX  --resume_mode=standard  --res_file=$RESUME_FILENAME   --prune_trained --prune_trained_pct=$PRUNE_PCT  --group_prune_strategy=taylor --prune_layer_mode=global --global_prune_normalization=by_layer  --set_prune_deemph   --prune_deemph_string=expand  --cuda   --output_level=warning --short_test_report  --verbose_prune_trained  --verbose_prune_trained_nodesc 
