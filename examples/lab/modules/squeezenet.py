...
def add_args(parser):
    ...
    parser.add_argument("--squeezenet_fork_entropy_thresholds_by_chunk",type=float,nargs="+" ) #ith element is entropy required to exit after ith chunk Note that the ith chunk will only be considered as a possible eexit if i is in squeezenet_fork_after_chunks
...
class NextFire(serialmodule.SerializableModule):
    ...
    def __init__(self, in_channels, num_squeeze, num_expand,skip,skipmode, groups=32, final_bn=False,stochastic_depth=False, survival_prob=1, shakedrop=False, shake_shake=False, shake_shake_mode= shake_shake.ShakeMode.IMAGE, proxy_ctx=None,proxy_mode = None, bypass_first_last=True):
        ...
        layer_dict["expand_conv"] =  first_last_wrapper(nn.Conv2d(num_squeeze, num_expand, kernel_size=1))
        if final_bn:
    ...
    def forward(self, x):
        ...
        for i,layer_chunk in enumerate(self.layer_chunk_list):
            ...
            if self.use_forking and i in self.fork_after_chunks:
                ...
                if not self.training and config.squeezenet_fork_early_exit:
                    ...
                    if entropy> config.fork_entropy_thresholds_by_chunk[i]:
        ...
        if self.use_forking and self.training:
            ...
            if score_list[0].is_cuda: #move all scores to the same device
                ...
                return score_list
        if not self.training and config.squeezenet_fork_early_exit and self.calculating_exit_proportions:
            self.total_exits+=1
        return x
...
def get_ext_props_on_sample(squeezenet,  loader):
    assert squeezenet.use_forking
